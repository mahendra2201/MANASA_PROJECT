...
const getSummary = async (req, res) => {
  try {
    ...
    const [recent_posts] = await db.query(`
    ...
    const [top_campaigns] = await db.query(`
      ...
      LEFT JOIN kpis k ON k.post_id = p.id
      GROUP BY c.id ORDER BY total_engagement DESC LIMIT 3
    `);

    const [platform_breakdown] = await db.query(`
    ...
    res.json({ success: true, data: { ...stats, recent_posts, top_campaigns, platform_breakdown } });
  } catch (err) {
